...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    @Query("SELECT r FROM Room r WHERE r.hotel.hotelCode = :hotelCode AND r.roomType = :roomType AND r.isDeleted = false AND r NOT IN (" + "SELECT b.room FROM BookingHotel b WHERE b.checkIn < :checkOut AND b.checkOut > :checkIn)")
    List<Room> findAvailableRoomsByHotelCodeAndDates(@Param("hotelCode") String hotelCode, @Param("checkIn") LocalDate checkIn, @Param("checkOut") LocalDate checkOut, @Param("roomType") String roomType);
